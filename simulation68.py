...
class simulation68(parent.simulationParent):

    def allWanderbutOne(self):
        ...
        for times in range(Max_TimeOut):
            ...
            for i in range(0,len(self.playerlist)):
                if(i==0):
                    self.playerlist[i].makeNoMove()
                    #print(self.playerlist[i].getPosition())
                else:
                    self.playerlist[i].makeOneMove()

                TotalPosition.add(tuple(self.playerlist[i].getPosition()))
    ...
    def DeterminedWanderTillMeet(self):
        Max_TimeOut = self.Max_TimeOut
    ...
    def wanderPolicy(self, policy_number = 0):
        #print(policy_number)
        if(policy_number ==1):
            self.allWanderbutOne()
        elif(policy_number == 2):
            self.DeterminedWanderTillMeet()
        else:
    ...
    def init_gui(self):
        ...
        row_idx += 1
        l4 = tk.Label(self.root, text=" Enter Strategy Number; 1- All wander but one | 2-Determined Move (Default Random)")
        l4.grid(column=1, row=row_idx)

        row_idx += 1
        self.policy_entry = tk.Entry(self.root, width=10)
        self.policy_entry.grid(column=1, row=row_idx)
    ...
    # calculate meet time statistics
    ...
    # update the meet time statistics on gui
    ...
    def animate_button(self):
        try:
            policyNbr = int(self.policy_entry.get())
            self.input_length_warning_clear()
        except:
            self.input_length_warning()

        self.wanderPolicy(policyNbr)
